...
class FrozenLakeEnv(discrete_env.DiscreteEnv):
    ...
    # action distribution per state is uniform?
    ...
    def __init__(self, desc=None, map_name="4x4",is_slippery=True):
        ...
        def to_s(row, col): # 4x4 table처럼 P에 접근하고 싶을 떄 사용
            return row*ncol + col
        def inc(row, col, a): # action을 취했을 때 movement
            if a==0: # left
                col = max(col-1,0)
            elif a==1: # down
            ...
            for col in range(ncol):
                ...
                for a in range(4):
                    ...
                    if letter in b'GH':
                        ...
                        if is_slippery:
                            for b in [(a-1)%4, a, (a+1)%4]: # 3가지 action을 상정(slippery)
                                newrow, newcol = inc(row, col, b) # s : a0:[(), (), ()]
