...
class Linear(DQN):
    ...
    def add_placeholders_op(self):
        ...
        channelhistory = state_shape[2] * config.state_history
        

        self.s = tf.placeholder(dtype = tf.uint8, shape=[None,img_height,img_width, channelhistory], name = 'state')
        self.a = tf.placeholder(dtype = tf.int32, shape=[None], name = 'action')
        self.r = tf.placeholder(dtype = tf.float32, shape=[None], name = 'reward')
        self.sp = tf.placeholder(dtype = tf.uint8, shape=[None,img_height,img_width, channelhistory], name = 'next_state')
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        x = tf.layers.flatten(state,name = scope)
    ...
    def add_update_target_op(self, q_scope, target_q_scope):
        ...
        update_var = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, q_scope)
        target_var = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, target_q_scope)
        assign = [tf.assign(target_var[i],update_var[i]) for i in range(len(target_var))]
